...
let testString = testInput.joined(separator: "\n")
...
func chapter4_1(from: String) -> Int {
    let parsedArray = parse(inputs: from.split(separator: "\n").map { String($0) })
    let minutesAsleep = parsedArray.map { ($0, $0.minutesAsleep) }
    let longestGuard = minutesAsleep.max(by: { $0.1 < $1.1 })!
    let maxMinute = longestGuard.0.asleepMost
    return longestGuard.0.id * maxMinute
}
//: Tests
//let testParsedArray = parse(inputs: testInput)
//let testMinutesAsleep = testParsedArray.map { ($0, $0.minutesAsleep) }
//// map with guard id for easier test
//let testVals = testMinutesAsleep.map { args -> (Int, Int) in
//    let (secu, mins) = args
//    return (secu.id, mins)
//}
//print("MinutesAsleep Test: \(testVals[0] == (10,50) && testVals[1] == (99,30))")
//print("AsleepMost Test: \(testMinutesAsleep[0].0.asleepMost == 24)")
//
//print("chapter4_1 Test: \(chapter4_1(from: testInput.joined(separator: "\n")) == 240)")

//: Calc the value with the actual data
let inputData = Helper.getInput(name: "Input", fileType: "txt")!

let result = chapter4_1(from: inputData)
print(result)
